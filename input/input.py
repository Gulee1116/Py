...
class Example(QWidget):
    ...
    def pictureStart(self):
        ...
        pictureName, _ = QFileDialog.getOpenFileName(
    ...
    def openFrame(self):
        if self.frame != []:
            ...
            ).scaled(self.upFileLabel.width(), self.upFileLabel.height())
            self.upFileLabel.setPixmap(QPixmap.fromImage(q_image))

    def closeEvent(self, event):
        reply = QMessageBox.question(
            self, "Message", "确定要退出吗?", QMessageBox.Yes | QMessageBox.No, QMessageBox.No
        )
